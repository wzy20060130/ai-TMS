...
async function sendCode() {
  ...
  try {
    const res = await proxy.$http.post('/send-code', {
      phone: form.phone,
    })
    // 换成访问 res.data
    if (res.data || res.data.code) {
      ...
      error.value = res.data?.message || '发送验证码失败'
    }
  } catch (e: any) {
    error.value = e.message || '请求发送验证码异常'
  } finally {
...
async function onSubmit() {
  ...
  if (mode.value === 'password') {
    ...
    try {
      const data = await proxy.$http.post('/login/password', {
    ...
    try {
      const data = await proxy.$http.post('/login/phone', {
        phone: form.phone,
        code: form.code,
      })
