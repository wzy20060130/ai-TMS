<template>
  <div class="min-h-screen flex flex-col lg:flex-row">
    ...
    <div class="lg:flex-1 h-64 lg:h-auto relative overflow-hidden bg-gray-50">
      ...
      <!-- 渐变叠加（提高右侧卡片对比度） -->
      <div
        class="absolute inset-y-0 right-0 w-2/5 pointer-events-none"
        style="
      ...
      <!-- 左上 logo（大屏显示） -->
      <div
        class="hidden lg:flex absolute top-8 left-8 items-center gap-3 bg-white/40 backdrop-blur-sm rounded-md px-3 py-2"
      >
        <div
          class="w-10 h-10 rounded-md flex items-center justify-center bg-brand text-white font-bold"
        >
          N
        </div>
        <div>
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-8">
          ...
          <div class="flex items-center gap-2 mb-4">
            <button
              :class="[
                ...
                mode === 'password' ? 'bg-brand text-white' : 'bg-gray-100 text-gray-600',
              ]"
              type="button"
              @click="mode = 'password'"
            >
            ...
            <button
              :class="[
                'px-3 py-1 rounded-md',
                mode === 'phone' ? 'bg-brand text-white' : 'bg-gray-100 text-gray-600',
              ]"
              type="button"
              @click="mode = 'phone'"
            >
          ...
          <form class="space-y-4" @submit.prevent="onSubmit">
            ...
            <div>
              <button
                type="submit"
                class="w-full py-2 bg-brand text-white rounded-md hover:opacity-95 transition"
              >
                登录
              </button>
            </div>
...
function _startCountdown(sec = 60) {
  countdown.value = sec;
...
async function onSubmit() {
  ...
  try {
    ...
    const errorMessage = e instanceof Error ? e.message : '登录异常';
